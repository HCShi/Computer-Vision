...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        hist_mag = zeros(pixelsPerHistogram,pixelsPerHistogram);
        hist_theta = zeros(pixelsPerHistogram, pixelsPerHistogram);
        for j=1:num_histograms
            for k=1:num_histograms
                 hist_mag = patch_mag(((j-1)*pixelsPerHistogram+1):(j*pixelsPerHistogram), ((k-1)*pixelsPerHistogram+1):(k*pixelsPerHistogram));
                 hist_theta = patch_theta(((j-1)*pixelsPerHistogram+1):(j*pixelsPerHistogram), ((k-1)*pixelsPerHistogram+1):(k*pixelsPerHistogram));
                 [histogram, angles] = ComputeGradientHistogram(num_angles, hist_mag, hist_theta);
                 feature = [feature,histogram];
            end
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:rows
        for j=1:cols
            index = ceil(gradient_angles(i,j) / angle_step);
            if index == 0
                index = 1;
            end
            histogram(index) = histogram(index) + gradient_magnitudes(i,j);
        end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    index = find(histogram == max(histogram));
    direction = angles(index(1));
